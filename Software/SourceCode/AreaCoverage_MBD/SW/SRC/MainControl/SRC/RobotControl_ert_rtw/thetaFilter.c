...
{
  boolean_T rtb_Relational_Operator_e2;
  int16_T rtb_Subtract;
  real32_T rtb_PT_p;
  real32_T rtb_Switch_f;
  real32_T rtb_KT;
  uint16_T rtb_Switch_n;
  real32_T rtb_XT_b;
   ...
   */
  rtb_Relational_Operator_e2 = (rtb_Subtract_0 > CAL_FilterResetThd);
   ...
   */
  if (rtb_Relational_Operator_e2 && (rtb_Subtract < CONST_ZeroDeg)) {
    rtb_Switch_n = CONST_ONE_UINT16;
  } else {
    rtb_Switch_n = CAL_fullCircle_DEG;
  }

  /* End of Switch: '<S37>/Switch' */

  /* Delay: '<S37>/Resettable_Delay' */
  if (rtb_Relational_Operator_e2 &&
      (RobotControl_PrevZCX.Resettable_Delay_Reset_ZCE != POS_ZCSIG)) {
    RobotControl_DW.icLoad = 1U;
  }

  RobotControl_PrevZCX.Resettable_Delay_Reset_ZCE = rtb_Relational_Operator_e2;
  if (RobotControl_DW.icLoad != 0) {
    RobotControl_DW.Resettable_Delay_DSTATE = rtb_Switch_n;
  }
   ...
   *  Sum: '<S40>/Add'
   */
  rtb_Switch_f = (real32_T)((RobotControl_B.leftVel1ms_mmPerSec -
  ...
  RobotControl_PrevZCX.Resettable_Delay1_Reset_ZCE = rtb_Relational_Operator_e2;
  if (RobotControl_DW.icLoad_o != 0) {
    RobotControl_DW.Resettable_Delay1_DSTATE = rtb_Switch_n;
  }
   ...
   *  Sum: '<S37>/Subtract2'
   */
  rtb_XT_b = ((real32_T)RobotControl_B.rawCompassValue - rtb_Switch_f) * rtb_KT
    + rtb_Switch_f;
   ...
   *  Switch: '<S41>/Switch1'
   */
  if ((int16_T)(real32_T)floor(rtb_XT_b - (real32_T)CONST_OVERFLOW_THD_UINT16) >
      rtb_Subtract) {
    rtb_XT_b -= (real32_T)CAL_fullCircle_DEG;
  } else {
    if ((int16_T)(real32_T)floor(rtb_XT_b + (real32_T)CONST_OVERFLOW_THD_UINT16)
       ...
       *  Sum: '<S41>/Add1'
       */
      rtb_XT_b += (real32_T)rtb_Subtract;
    }
  }
  ...
  if (RobotControl_DW.Unit_Delay3_DSTATE_d) {
    rtb_Switch_f = RobotControl_B.Data_Type_Conversion_a;
  } else if (rtb_XT_b > 360.0F) {
    /* Saturate: '<S37>/Saturation' */
    rtb_Switch_f = 360.0F;
  } else if (rtb_XT_b < 0.0F) {
    /* Saturate: '<S37>/Saturation' */
    rtb_Switch_f = 0.0F;
  } else {
    /* Saturate: '<S37>/Saturation' */
    rtb_Switch_f = rtb_XT_b;
  }

  /* End of Switch: '<S36>/Switch' */

  /* Saturate: '<S36>/Saturation' */
  if (rtb_Switch_f > CAL_fullCircle_DEG) {
    RobotControl_B.theta_Deg = CAL_fullCircle_DEG;
  } else if (rtb_Switch_f < CONST_ZERO_UINT16) {
    RobotControl_B.theta_Deg = CONST_ZERO_UINT16;
  } else {
    RobotControl_B.theta_Deg = rtb_Switch_f;
  }
  ...
  RobotControl_DW.Resettable_Delay_DSTATE = rtb_XT_b;
