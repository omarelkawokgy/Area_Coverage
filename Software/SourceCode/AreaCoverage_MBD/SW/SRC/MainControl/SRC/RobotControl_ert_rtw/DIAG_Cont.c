...
#define RobotCont_IN_NO_ACTIVE_CHILD_ec ((uint8_T)0U)
...
void RobotControl_DIAG_Cont_Init(void)
{
  ...
  RobotControl_DW.is_c34_RobotControlLib = RobotCont_IN_NO_ACTIVE_CHILD_ec;
  RobotControl_DW.tempDist_c = 0.0F;
  ...
  RobotControl_DW.is_c12_RobotControlLib = RobotCont_IN_NO_ACTIVE_CHILD_ec;
...
void RobotControl_DIAG_Cont(void)
{
  ...
  if (RobotControl_DW.temporalCounter_i1_a < 15U) {
  ...
  if (RobotControl_DW.is_active_c34_RobotControlLib == 0U) {
    ...
    switch (RobotControl_DW.is_c34_RobotControlLib) {
     ...
     case RobotControl_IN_moveRefresh:
      ...
      if (rtb_moveFlag) {
        /* Transition: '<S47>:8' */
        RobotControl_DW.tempDist_c = RobotControl_B.Data_Type_Conversion;
